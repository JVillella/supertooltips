...
public class ToolTipView extends LinearLayout implements ViewTreeObserver.OnPreDrawListener, View.OnClickListener {
    ...
    private void applyToolTipPosition() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
            mTopPointerView.setAlpha(showBelow ? 1 : 0);
            mBottomPointerView.setAlpha(showBelow ? 0 : 1);
        } else {
    ...
    public void remove() {
        ...
        if (mToolTip.getAnimationType() == ToolTip.AnimationType.NONE) {
            ...
            Collection<Animator> animators = new ArrayList<>(5);
            if (mToolTip.getAnimationType() == ToolTip.AnimationType.FROM_MASTER_VIEW) {
                animators.add(ObjectAnimator.ofFloat(this, TRANSLATION_Y_COMPAT, getY(), mRelativeMasterViewY + mView.getHeight() / 2 - getHeight() / 2));
                animators.add(ObjectAnimator.ofFloat(this, TRANSLATION_X_COMPAT, getX(), mRelativeMasterViewX + mView.getWidth() / 2 - mWidth / 2));
            } else {
